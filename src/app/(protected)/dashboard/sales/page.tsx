...
type Sale = {
  ...
  outlet_id: string; // Needed for editing
  outlet_name: string;
...
export default function AdminSalesPage() {
  ...
  // We use 'any' here to handle the various form fields flexibly
  ...
  // Real-time listener updates the list automatically when DB changes
  useEffect(() => {
    const channel = supabase.channel('admin-sales-channel')
      .on('postgres_changes', { event: '*', schema: 'public', table: 'customers' }, () => fetchSales())
      .subscribe();
    return () => { supabase.removeChannel(channel); };
  }, [fetchSales]);

  // --- Totals Calculation (Live based on 'sales' state) ---
  const activeSales = useMemo(() => sales.filter((s) => !!s.check_out_time), [sales]);
  
  const totalSales = useMemo(() => activeSales.reduce((sum, s) => sum + (s.took_package ? s.package_amount : s.amount_paid), 0), [activeSales]);
  
  const totalCashSales = useMemo(() => activeSales
    .filter(s => s.payment_method === 'cash')
    .reduce((sum, s) => sum + (s.took_package ? s.package_amount : s.amount_paid), 0), [activeSales]);

  const totalUpiSales = useMemo(() => activeSales
    .filter(s => s.payment_method === 'card' || s.payment_method === 'upi')
    .reduce((sum, s) => sum + (s.took_package ? s.package_amount : s.amount_paid), 0), [activeSales]);

  const totalPackageSales = useMemo(() => activeSales
    .filter(s => s.took_package)
    .reduce((sum, s) => sum + (s.package_amount || 0), 0), [activeSales]);
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      const dataToExport = data.map((sale: any) => {
        ...
        if (sale.took_package) service = `New Package - ${sale.treatment}`;
        else if (sale.is_package_customer) service = `Package Redemption - ${sale.treatment}`;
        ...
        return {
          ...
          Room: sale.room || 'N/A',
          'Check-in': formatTime(sale.check_in_time),
          'Check-out': formatTime(sale.check_out_time),
        };
      });
      
      const outletName = selectedOutletId === 'all' ? 'AllOutlets' : OUTLETS.find(o => o.id === selectedOutletId)?.name || 'Outlet';
      exportToExcel(dataToExport, `Sales_${outletName}_${startDate}.xlsx`);
    } catch (err: any) {
      console.error(err);
      alert('Export failed');
    } finally {
  ...
  const handleDelete = async (id: string) => {
    if (!confirm('PERMANENTLY DELETE record? Cannot undo.')) return;
  ...
  const handleSaveEdit = async (e: FormEvent) => {
    ...
    try {
      ...
      if (editingSale?.took_package) {
        ...
        // Don't touch amount_paid, keep it 0 for package sales
      } else {
      ...
      await fetchSales(); // Immediate refresh to update totals
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white p-4 rounded-xl shadow-sm grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 items-end">
        <div>
          ...
          <select value={selectedOutletId} onChange={(e) => setSelectedOutletId(e.target.value)} className="w-full px-3 py-2 border border-gray-300 rounded-lg">
        ...
        <button className="w-full px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700" onClick={handleExport}>
          {isExporting ? 'Exporting...' : 'Export to Excel'}
        </button>
      </div>

      {/* Totals Cards */}
      <div className="bg-white p-6 rounded-xl shadow-sm grid grid-cols-1 md:grid-cols-4 gap-4 divide-x">
        <div className="px-4">
          ...
          <p className="text-gray-500 text-sm">{activeSalesCount} sessions</p>
        </div>
        <div className="px-4">
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full divide-y divide-gray-200">
            <thead className="bg-gray-50">
              <tr>
                {['Customer', 'Outlet', 'Date', 'Service', 'Amount', 'Payment', 'Time', 'Therapist', 'Room', 'Actions'].map(h => (
            ...
            <tbody className="divide-y divide-gray-200">
              {loading ? <tr><td colSpan={10} className="p-6 text-center">Loading...</td></tr> : 
               sales.map(sale => (
                <tr key={sale.id} className={sale.check_out_time ? 'bg-gray-50' : 'bg-white'}>
                  <td className="px-3 py-2">
                    <div className="text-xs font-bold text-gray-900">{sale.name}</div>
                    <div className="text-xs text-gray-500">{sale.mobile}</div>
                  </td>
                  <td className="px-3 py-2 text-xs">{sale.outlet_name}</td>
                  <td className="px-3 py-2 text-xs">{toInputDate(sale.date)}</td>
                  <td className="px-3 py-2 text-xs max-w-xs">{formatService(sale)}</td>
                  ...
                  <td className="px-3 py-2 text-xs">{formatPaymentMethod(sale.payment_method)}</td>
                  <td className="px-3 py-2 text-xs">
                    <div>In: {formatTime(sale.check_in_time)}</div>
                    {sale.check_out_time && <div>Out: {formatTime(sale.check_out_time)}</div>}
                  </td>
                  <td className="px-3 py-2 text-xs">{sale.therapist_name || '—'}</td>
                  <td className="px-3 py-2 text-xs">{sale.room || '—'}</td>
                  <td className="px-3 py-2">
                    <div className="flex gap-2">
                      {/* --- EDIT BUTTON --- */}
                      <button onClick={() => handleOpenEdit(sale)} className="px-2 py-1 bg-blue-100 text-blue-700 rounded hover:bg-blue-200 text-xs">Edit</button>
                      {!sale.check_out_time && (
                        <button onClick={() => handleCheckOut(sale.id)} className="px-2 py-1 bg-green-100 text-green-700 rounded hover:bg-green-200 text-xs">Out</button>
                      )}
                      <button onClick={() => handleDelete(sale.id)} className="px-2 py-1 bg-red-100 text-red-700 rounded hover:bg-red-200 text-xs">Del</button>
                    </div>
                  </td>
                </tr>
              ))}
          ...
          <form onSubmit={handleSaveEdit} className="bg-white rounded-xl shadow-xl w-full max-w-lg p-6 space-y-4 max-h-[90vh] overflow-y-auto">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <input type="text" value={editForm.name} onChange={e => setEditForm({...editForm, name: e.target.value})} className="w-full p-2 border rounded" required />
              </div>
              <div>
                <label className="text-xs font-semibold text-gray-500 uppercase">Mobile</label>
                <input type="text" value={editForm.mobile} onChange={e => setEditForm({...editForm, mobile: e.target.value})} className="w-full p-2 border rounded" required />
              </div>
              <div>
                <label className="text-xs font-semibold text-gray-500 uppercase">Outlet</label>
                <select value={editForm.outlet_id} onChange={e => setEditForm({...editForm, outlet_id: e.target.value})} className="w-full p-2 border rounded bg-white">
                  {OUTLETS.map(o => <option key={o.id} value={o.id}>{o.name}</option>)}
                </select>
              </div>
              <div>
            ...
            <div>
              <label className="text-xs font-semibold text-gray-500 uppercase">Treatment</label>
              <input type="text" value={editForm.treatment} onChange={e => setEditForm({...editForm, treatment: e.target.value})} className="w-full p-2 border rounded" required />
            </div>

            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="text-xs font-semibold text-gray-500 uppercase">Amount (₹)</label>
                <input type="number" value={editForm.amount} onChange={e => setEditForm({...editForm, amount: e.target.value})} className="w-full p-2 border rounded" required />
              </div>
              <div>
                <label className="text-xs font-semibold text-gray-500 uppercase">Payment Method</label>
                <select value={editForm.payment_method} onChange={e => setEditForm({...editForm, payment_method: e.target.value})} className="w-full p-2 border rounded bg-white">
            ...
            <div className="grid grid-cols-3 gap-4">
              <div>
                ...
                <input type="text" value={editForm.therapist_name} onChange={e => setEditForm({...editForm, therapist_name: e.target.value})} className="w-full p-2 border rounded" />
              </div>
              <div>
                <label className="text-xs font-semibold text-gray-500 uppercase">Room</label>
                <input type="text" value={editForm.room} onChange={e => setEditForm({...editForm, room: e.target.value})} className="w-full p-2 border rounded" />
              </div>
              <div>
                <label className="text-xs font-semibold text-gray-500 uppercase">Duration (Hrs)</label>
                <input type="number" step="0.1" value={editForm.session_hours} onChange={e => setEditForm({...editForm, session_hours: e.target.value})} className="w-full p-2 border rounded" />
              </div>
            </div>
            ...
            <div className="flex justify-end gap-3 pt-2">
              <button type="button" onClick={() => setIsEditModalOpen(false)} className="px-4 py-2 bg-gray-200 rounded hover:bg-gray-300 text-gray-800">Cancel</button>
