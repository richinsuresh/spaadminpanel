...
export default function PackagesPage() {
  ...
  const normalizeRow = (row: any): PackageCustomer => {
    // Safely parse numbers and strings and provide defaults
  ...
  const fetchPackages = useCallback(async () => {
    ...
    setErrorMsg(null);
    try {
      // Try to fetch packages; order by created_at if present.
      ...
      if (error) {
        // If ordering by created_at errors (missing column), try without order
  ...
  useEffect(() => {
    ...
    const interval = setInterval(fetchPackages, 30000); // 30s auto-refresh
                ...
                {filteredPackages.map((customer) => (
                  <tr key={customer.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                      {customer.expiry_date ? (new Date(customer.expiry_date).toLocaleDateString('en-IN')) : '—'}
                    </td>
