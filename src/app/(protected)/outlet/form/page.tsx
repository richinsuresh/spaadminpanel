...
export default function OutletClientForm() {
  ...
  const [outletName, setOutletName] = useState('Loading...'); // State to hold the current outlet name, fetched from cookie
  const [formData, setFormData] = useState<FormData>({ // State for the rest of the form data
    ...
    outlet: 'Loading...', // Will be updated from cookie
  });
  ...
  // Get current outlet name from cookie on component mount
  useEffect(() => {
    ...
    if (outletId) {
      ...
      if (outlet) {
        ...
        // Set the outlet in formData as well, as it's needed for submission
        setFormData(prev => ({ ...prev, outlet: currentOutletName }));
      } else {
         setOutletName('Unknown Outlet'); // Handle case where cookie ID is invalid
         setFormData(prev => ({ ...prev, outlet: 'Unknown Outlet' }));
      }
    } else {
        // Handle case where cookie is missing (should be caught by layout, but good practice)
        console.error("Outlet ID cookie missing!");
        setOutletName('Error: Outlet not found');
        setFormData(prev => ({ ...prev, outlet: 'Error: Outlet not found' }));
        // Potentially redirect here if critical: router.replace('/outlet-login');
    }
  ...
  // Effect to perform client lookup based on mobile number
  useEffect(() => {
  ...
  }, [mobile]); // Re-run effect when mobile state changes
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Determine final amount paid (0 if package related)
      ...
      const response = await fetch('/api/submit', {
        ...
        body: JSON.stringify({ 
            ...formData, 
            mobile: mobile, // Use the separate mobile state for submission
            outlet: outletName, // Ensure the current outlet name is sent
      ...
      if (response.ok) {
        ...
        // After 1.5 seconds, refresh data on the target page and redirect
        setTimeout(() => {
            router.refresh(); // Crucial for updating dashboard data
            router.push('/outlet/dashboard'); // Redirect Outlet staff to Outlet Dashboard
        }, 1500); 
        ...
        setFormData({
          ...
          isPackageCustomer: false, outlet: outletName, // Keep current outlet
        });
      } else {
         // Handle API errors
  ...
  // Determine if the Amount Paid field should be visible
  ...
  return (
    <div className="max-w-3xl mx-auto p-6 bg-white rounded-xl shadow-md mt-8 relative">
      ...
      <button
        type="button"
        // Close button goes to the Outlet Dashboard
      ...
      {clientInfo && (
        <div className={`mb-6 p-4 rounded-lg border ${
          clientInfo.status === 'active' ? 'bg-green-50 border-green-200' : 'bg-yellow-50 border-yellow-200' // Use yellow for expired/none
        }`}>
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          ...
          <div>
            ...
            {/* Display outlet name, but disable selection */}
            <input
              name="outlet"
              type="text"
              value={outletName}
